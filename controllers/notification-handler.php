...
if ($transaction == 'capture') {
    ...
    include "../../controllers/dbs.php";
    updateQuery("UPDATE donation SET order_status=3 WHERE order_id=$order_id");
} else if ($transaction == 'pending') {
    // TODO set payment status in merchant's database to 'Pending'
    // echo "Waiting customer to finish transaction order_id: " . $order_id . " using " . $type;
    include "../../controllers/dbs.php";
    updateQuery("UPDATE donation SET order_status=2 WHERE order_id=$order_id");
} else if ($transaction == 'deny') {
    // TODO set payment status in merchant's database to 'Denied'
    // echo "Payment using " . $type . " for transaction order_id: " . $order_id . " is denied.";
    include "../../controllers/dbs.php";
    updateQuery("UPDATE donation SET order_status=1 WHERE order_id=$order_id");
